...
public class MainActivity extends Activity {
    ...
    private void draw() {
        ...
        input.setOnFocusChangeListener((v, focused) -> { if (focused) { Runnable reveal = () -> { Rect rect = new Rect(); input.getDrawingRect(rect); scroll.offsetDescendantRectToMyCoords(input, rect); rect.bottom += dp(220); scroll.requestChildRectangleOnScreen(input, rect, true); }; input.postDelayed(reveal, 300); input.postDelayed(reveal, 800); } });
        ...
        input.addTextChangedListener(new TextWatcher() { public void beforeTextChanged(CharSequence s,int a,int b,int c){} public void onTextChanged(CharSequence s,int a,int b,int c){ ExpenseParser.Result p=ExpenseParser.parse(s.toString()); preview.setText(p.amount>0 ? p.category+"  •  "+Format.money(p.amount) : "Gemini sẽ tự phân loại theo danh mục"); } public void afterTextChanged(Editable e){} });
        ...
        for (int i=items.length()-1; i>=0; i--) try {
            ...
            LocalDate itemDate = LocalDate.parse(date);
    ...
    private void showDateFilter() {
        if (filterStartDate != null && filterEndDate == null) showEndDatePicker(); else showStartDatePicker();
    }

    private void showStartDatePicker() {
        Calendar today = Calendar.getInstance(); LocalDate selected = filterStartDate == null ? LocalDate.now() : LocalDate.parse(filterStartDate);
        DatePickerDialog dialog = new DatePickerDialog(this, (view, year, month, day) -> { filterStartDate=String.format(Locale.ROOT,"%04d-%02d-%02d",year,month+1,day);filterEndDate=null;Toast.makeText(this,"Chọn tiếp ngày kết thúc",Toast.LENGTH_SHORT).show();showEndDatePicker(); }, selected.getYear(), selected.getMonthValue()-1, selected.getDayOfMonth());
        Calendar minimum=Calendar.getInstance();minimum.add(Calendar.MONTH,-5);minimum.set(Calendar.DAY_OF_MONTH,1);dialog.getDatePicker().setMinDate(minimum.getTimeInMillis());dialog.getDatePicker().setMaxDate(today.getTimeInMillis());dialog.show();
    }

    private void showEndDatePicker() {
        Calendar today=Calendar.getInstance();LocalDate start=LocalDate.parse(filterStartDate);DatePickerDialog dialog=new DatePickerDialog(this,(view,year,month,day)->{String picked=String.format(Locale.ROOT,"%04d-%02d-%02d",year,month+1,day);if(LocalDate.parse(picked).isBefore(start)){filterEndDate=filterStartDate;filterStartDate=picked;}else filterEndDate=picked;draw();},start.getYear(),start.getMonthValue()-1,start.getDayOfMonth());Calendar minimum=Calendar.getInstance();minimum.add(Calendar.MONTH,-5);minimum.set(Calendar.DAY_OF_MONTH,1);dialog.getDatePicker().setMinDate(minimum.getTimeInMillis());dialog.getDatePicker().setMaxDate(today.getTimeInMillis());dialog.show();
    }
    ...
    private void expenseDialog(JSONObject o) {
        ...
        new AlertDialog.Builder(this).setTitle("Chi tiết khoản chi").setMessage(message).setNegativeButton("Đóng", null).setNeutralButton("ĐỔI NGUỒN", (d, w) -> editExpenseSource(o)).setPositiveButton("ĐỔI MỤC TIÊU", (d, w) -> editExpenseCategory(o)).show();
    }
    ...
    private void editExpenseSource(JSONObject o){java.util.List<String> values=store.sources();int checked=Math.max(0,values.indexOf(o.optString("source", "Tiền Mặt")));new AlertDialog.Builder(this).setTitle("Đổi nguồn chi").setSingleChoiceItems(values.toArray(new String[0]),checked,(d,which)->{String source=values.get(which);if(store.updateSource(o,source)){NotificationHelper.refresh(this);Toast.makeText(this,"Đã chuyển sang "+source,Toast.LENGTH_SHORT).show();draw();}d.dismiss();}).setNegativeButton("Hủy",null).show();}
